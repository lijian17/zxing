 ...
 * 
 * @author lijian
 * @date 2017-9-3 下午7:34:55
 */
final class LoadPackagesAsyncTask extends
		AsyncTask<Object, Object, List<AppInfo>> {
	...
	protected List<AppInfo> doInBackground(Object... objects) {
		...
		Iterable<ApplicationInfo> appInfos = packageManager
		...
		for (PackageItemInfo appInfo : appInfos) {
			...
			if (!isHidden(packageName)) {
				...
				if (label != null) {
					labelsPackages.add(new AppInfo(packageName, label
							.toString(), icon));
				}
			}
		}
	...
	protected void onPostExecute(final List<AppInfo> results) {
		ListAdapter listAdapter = new ArrayAdapter<AppInfo>(activity,
				R.layout.app_picker_list_item, R.id.app_picker_list_item_label,
				results) {
			@Override
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (icon != null) {
					((ImageView) view
							.findViewById(R.id.app_picker_list_item_icon))
